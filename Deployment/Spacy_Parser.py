...
# Uses spacy to parse product names through word embedding 
# This automatically transforms the data from the version without parsed products to the version with parsed products.
class SpacyParser(BaseEstimator, TransformerMixin):                                  
    def fit(self, X, y=None):                                   
        return self                                              
    def transform(self, df):                                    
        if any(df['product'].str.contains(',')):                
